...
class Actor:
    def __init__(
        ...
        if model_path == None:
            ...
            self.log_std = tf.Variable(
                tf.ones([300, action_shape[0]]) * log_std_init,
                trainable=True,
                name="log_std",
            )
            self.exploration_mat = tf.Variable(
                tf.zeros_like(self.log_std), trainable=False, name="exploration_mat"
            )
            # print(self.log_std)
    ...
    def predict(self, x, with_logprob=True, deterministic=False):
        ...
        if deterministic:
            ...
            if with_logprob:
                variance = tf.matmul(
                    tf.square(latent_sde), tf.square(tf.exp(self.log_std))
                )
