...
if __name__ == "__main__":
    ...
    my_parser = argparse.ArgumentParser(prog='python3 training.py', description='RL training toolkit', formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    ...
    my_parser.add_argument('-lr', '--learning_rate', type=float, help='Learning rate', default=7.3e-4)
    ...
    # replay buffer
    rpm = ReplayBuffer(args.environment, env.observation_space.shape, env.action_space.shape, args.replay_size)
    ...
    while total_steps < args.max_steps:
        ...
        while not done:
            ...
            if total_steps > args.update_after and len(rpm) > args.batch_size and (total_steps % args.update_every) == 0:
                agent.update(rpm, args.batch_size, args.update_every, logging_wandb=args.wandb)
